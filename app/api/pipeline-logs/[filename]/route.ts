/**
 * Next.js API route for fetching a specific pipeline log file
...
export async function GET(
  ...
  try {
    ...
    const response = await fetch(`${BACKEND_URL}/api/pipeline-logs/${filename}`, {
      method: 'GET',
      headers: {
    ...
    if (!response.ok) {
      ...
      return Response.json(
        {
          error: errorData.error || 'Pipeline log fetch failed',
          message: errorData.message || 'Failed to fetch pipeline log',
        },
