/**
 * Next.js API route for processing a pipeline log file
...
export async function POST(
  ...
  try {
    ...
    const response = await fetch(`${BACKEND_URL}/api/pipeline-logs/${filename}/process`, {
      method: 'POST',
      headers: {
    ...
    if (!response.ok) {
      ...
      return Response.json(
        {
          error: errorData.error || 'Pipeline log processing failed',
          message: errorData.message || 'Failed to process pipeline log',
        },
